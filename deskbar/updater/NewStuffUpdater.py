...
class NewStuffUpdater:
	...
	def check_all(self):
		self.newstuff.Refresh(reply_handler=self.on_newstuff_refreshed, error_handler=global_error_handler)
	
	def on_newstuff_refreshed(self):
		plugins = [(self.id_for_module_context(context), '0.0.0') for context in self.module_list]
	...
	def on_newstuff_updated(self, plugin_id):
		...
		mod_ctx.update_infos = (False, None)
		self.module_list.module_changed(mod_ctx)	
		
	def install(self, mod_ctx):
		#self.newstuff.Update(mod_ctx.id, reply_handler=lambda: self.on_module_updated(mod_ctx), error_handler=global_error_handler)
		print 'Should install:', mod_ctx
		pass
